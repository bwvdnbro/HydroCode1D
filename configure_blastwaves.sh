...
if [ $# -lt 1 ]
then
  echo "Usage: ./configure_blastwaves.sh <PATH TO SOURCE FOLDER>"
  exit 1
fi

cmake -DCMAKE_BUILD_TYPE=Release -Drmin_in_m=0. -Drmax_in_m=1. -Dncell=1000 \
      -Dgamma=1.4 -Dmaxtime_in_s=0.038 -Dnumber_of_snaps=1 -Deos=EOS_IDEAL \
      -Dboundaries=BOUNDARIES_REFLECTIVE -Dpotential=POTENTIAL_NONE \
